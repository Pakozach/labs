...
byte *str_to_iv (char *s, int enc_type, int *block_size){
    *block_size = get_block_size(enc_type);
    ...
    byte *iv = calloc(*block_size, sizeof (byte));

    for (int i=0; i<(*block_size); i++){
        char *c1=strchr(HEX_CHARS,s[2*i]);
        char *c2=strchr(HEX_CHARS,s[2*i+1]);
        if ((c1 != NULL) && (c2 != NULL))
            iv[i] = (c1-HEX_CHARS)*16 + (c2-HEX_CHARS);
        else{
            free(iv);
            return NULL;
        }
...
int str_to_nonce (char *s, t_nonce nonce){
    ...
    for (int i=0; i<NONCE_LENGTH; i++){
        ...
        char *c2=strchr(HEX_CHARS,s[2*i+1]);
        if ((c1 != NULL) && (c2 != NULL))
            nonce[i] = (c1-HEX_CHARS)*16 + (c2-HEX_CHARS);
        else{
            return 0;
        }
    }
...
int encode_f (byte *text, int n, t_password psw, int hash_type, int enc_type, t_nonce nonce, byte *iv, int block_size, FILE *f_out){
    int res = 0;
...
int encode (byte *text, int n, t_password psw, int hash_type, int enc_type, t_nonce nonce, byte *iv, int block_size, char *dir_out){
    int res = 0;

    char *file_path = get_enc_file_path(dir_out, psw, hash_type, enc_type);
    FILE *f_out = fopen(file_path,"wb+");
    if (f_out) {
        res = encode_f(text, n, psw, hash_type, enc_type, nonce, iv, block_size, f_out);
        fclose(f_out);
    } else {
...
int read_file_data(FILE *f_in, t_buffer *buf_in){
    int res = 1;
    byte b;
...
void do_generate(t_password psw, int hash_type, int enc_type, char *dir_out){
    ...
    encode(text, strlen(text), psw, hash_type, enc_type, nonce, iv, block_size, dir_out);
}
...
int encode_file (char *path_in, char *path_out, t_password psw, int hash_type, int enc_type, t_nonce nonce, byte *iv, int block_size){
    ...
    if (f_in) {
        ...
        if (f_out) {
            if (read_file_data(f_in, buf_in)){
                res = encode_f(buf_in->data, buf_in->len, psw, hash_type, enc_type, nonce, iv, block_size, f_out);
            } else
                printf("File data too big %s\n",path_in);
            fclose(f_out);
        } else
...
int decode_file (char *path_in, char *path_out, t_password psw) {
    ...
    if (f_in) {
        ...
        if (read_enc_file(f_in, &hash_type, &enc_type, nonce, &iv, &block_size, buf_in)){
            ...
            if (!(k==NULL)) {
                if (decode_buf(buf_in, buf_out, enc_type, k, iv)) {
                    if (buf_have_zero8(buf_out)) {

                        FILE *f_out = fopen(path_out, "wb+");
                        if (f_out) {
                            fwrite(buf_out->data + ZERO_HEAD_LENGTH, buf_out->len - ZERO_HEAD_LENGTH, 1, f_out);
                            fclose(f_out);
                        } else
                            printf("Can not open output file %s\n", path_out);
                    } else
                        printf("Bad password to decode file data\n");
                } else
                    printf("Can not decode file data\n");
                free(k);
            }
